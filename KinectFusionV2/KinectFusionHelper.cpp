...
HRESULT WriteTexturedeAsciiObjMeshFile(INuiFusionColorMesh *mesh, char* lpOleFileName, bool flipYZ, NUI_FUSION_IMAGE_FRAME *pTexture, const std::vector<Vector3> &texcoords)
{
	...
	std::wstring textureFilenamePath = std::wstring_convert<std::codecvt_utf8<wchar_t>>().from_bytes(std::string(filenamePath + textureFilename).c_str());
	...
	hr = SaveBMPFile(textureFilenamePath.c_str(), fusionColorBuffer->pBits, pTexture->width, pTexture->height);
